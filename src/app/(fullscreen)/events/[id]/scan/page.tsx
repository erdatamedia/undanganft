...
export default function EventScanPage() {
  ...
  const redirectToLogin = useCallback(() => {
    window.location.href = `/auth/login?redirect=/events/${eventId}/scan`;
  }, [eventId]);
  ...
  useEffect(() => {
    ...
    const onVis = () => { if (!document.hidden) ensureFocus(); };
  ...
  const parseScannerInput = (raw: string): ScanPayload => {
    ...
    if (!trimmed) throw new Error("Kode undangan kosong.");
    if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
      try {
        ...
        if (inviteIndex >= 0 && parts[inviteIndex + 1]) {
          return { inviteId: decodeURIComponent(parts[inviteIndex + 1]) };
        }
      } catch { /* fallthrough */ }
    }
    if (trimmed.startsWith("{")) {
      const parsed = JSON.parse(trimmed) as ScanPayload;
      if (!parsed.inviteId) throw new Error("Payload scanner tidak memuat inviteId.");
      return parsed;
    }
  ...
  const confirmAttendance = useCallback(async (payload: ScanPayload) => {
    ...
    try {
      ...
      try {
        const detailRes = await fetch(`/api/attendees/${encodeURIComponent(normalizedId)}`, {
          credentials: "include",
          cache: "no-store",
        });
        if (detailRes.ok) latest = (await detailRes.json()) as AttendeeRecord;
      } catch { /* fallback */ }
      setScanned(latest);
      setStatus("success");
  ...
  const submitManualPayload = useCallback(async (value: string) => {
    ...
    const now = Date.now();
    if (value === lastSubmittedRef.current && now - lastSubmitAtRef.current < 800) return;
    try {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.ctrlKey || event.metaKey || event.altKey) { event.preventDefault(); return; }
    ...
    const forceRefocus = () => manualInputRef.current?.focus({ preventScroll: true });
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 pb-12 pt-8">
        <header className="flex flex-col gap-4 text-center">
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Kembali ke detail acara
          </Link>
          ...
          <p className="text-sm text-slate-300">
            ...
            Kamera internal tersedia sebagai cadangan.
          </p>
        </header>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold transition ${
                scanMode === "hardware" ? "bg-emerald-500 text-white" : "bg-slate-800 text-slate-300"
              }`}
            >
              <Wifi className="h-4 w-4" /> Scanner Omni
            </button>
            <button
              type="button"
              onClick={() => setScanMode("camera")}
              className={`flex flex-1 items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold transition ${
                scanMode === "camera" ? "bg-emerald-500 text-white" : "bg-slate-800 text-slate-300"
              }`}
            >
              <Smartphone className="h-4 w-4" /> Kamera Internal
            </button>
          </div>
        </div>
          ...
          <div className="overflow-hidden rounded-3xl bg-slate-900/60 shadow-xl backdrop-blur">
            <div className="relative aspect-[3/4] w-full">
              <Scanner
                ...
                onError={(err) => { setStatus("error"); setMessage((err as Error).message); }}
        ...
        <div className={`rounded-3xl border px-5 py-4 text-sm ${
          status === "error" ? "border-rose-500/40 bg-rose-500/10 text-rose-100"
          : status === "success" ? "border-emerald-400/40 bg-emerald-400/10 text-emerald-100"
        ...
        {scanned && (
          <div className="rounded-3xl border border-emerald-400/40 bg-emerald-400/5 p-6 text-sm text-emerald-50">
            <div className="flex items-center gap-2 text-emerald-200">
              <CheckCircle2 className="h-5 w-5" /> QR tervalidasi
            </div>
            <p className="mt-3 text-2xl font-semibold text-white">{scanned.name}</p>
            ...
            <div className="mt-4 grid gap-3 text-xs text-slate-200 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
                <p className="uppercase tracking-[0.2em] text-slate-400">npm</p>
                <p className="text-base text-white">{scanned.npm}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
                <p className="uppercase tracking-[0.2em] text-slate-400">kursi</p>
                <p className="text-base text-white">{scanned.seat}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
                <p className="uppercase tracking-[0.2em] text-slate-400">waktu scan</p>
                <p className="text-base text-white">
                  {new Date(scanned.confirmedAt ?? scanned.updatedAt).toLocaleString("id-ID")}
                </p>
              </div>
            </div>
          </div>
        ...
        <form onSubmit={handleManualSubmit} className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
          <p className="text-sm text-slate-300">
            Kolom utama scanner Omni — hasil scan terbaca otomatis sebagai teks.
          </p>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <input
              ...
              placeholder='ID peserta atau {"inviteId":"..."}'
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === "NumpadEnter" || e.key === "Tab") {
                  e.preventDefault();
                  void submitManualPayload((e.currentTarget as HTMLInputElement).value);
                }
              }}
              onFocus={(e) => e.currentTarget.select()}
              autoCapitalize="off" autoCorrect="off" autoComplete="off" spellCheck={false} inputMode="text"
            />
            <button
              type="submit"
              disabled={!manualCode}
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-400 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              <QrCode className="h-4 w-4" /> Verifikasi
            </button>
          </div>
        </form>
        ...
        {lastPayload && (
          <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 text-sm text-slate-300">
            <div className="flex items-center gap-2 text-slate-100">
              <Shield className="h-4 w-4" /> Log terakhir
            </div>
            <pre className="mt-3 rounded-2xl bg-slate-950/60 p-3 text-xs text-slate-200">
